...
void IntBSTree::pushInOrder(BTNode* p, std::list<int> outList) const {
    /*if (p == nullptr) return;
    if (p->left != nullptr) {
        outList.push_back(p->left->data);
    }
    outList.push_back(p->data);
    if (p->right != nullptr) {
        outList.push_back(p->right->data);
    }*/
    if(!p) return;
    pushInOrder(p->left, outList);
    outList.push_back(p->data);
    pushInOrder(p->right, outList);
}

std::list<int> IntBSTree::getOrderedList() const {
    /*BTNode* ptr {root};
    BTNode* prev {nullptr};*/
    std::list<int> OList; // ordered list from smallest to largest

    // reach parent of smallest node
    /*while (ptr->left->left != nullptr) {
        prev = ptr;
        ptr = ptr->left;
    }
    OList.push_back(ptr->data); // add smallest node to list

    // add parent node and node to right of parent node if it exists
    */
    pushInOrder(root, OList);
    return OList;
}